...
function displayEditForm(id) {
    const item = bakeries.find(item => item.id === id);
    if (!item) {
...
function _displayCount(itemCount) {
    const name = (itemCount === 1) ? 'Bakery' : 'bakeries kinds';

    document.getElementById('counter').innerText = `${itemCount} ${name}`;
}

function _displayItems(data) {
    const tBody = document.getElementById('bakeries');
    tBody.innerHTML = '';

    _displayCount(data.length);

    const button = document.createElement('button');
    const arrCategory = ["Cheesecake", "ChocolateCake", "CarrotCake", "VanillaCake"];
